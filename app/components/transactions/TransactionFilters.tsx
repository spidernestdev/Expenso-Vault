...
import { categoryAPI } from "@/app/lib/api"; // ✅ use your api
import { 
  Filter, 
  X, 
  Calendar, 
...
export default function TransactionFilters({ filters, setFilters }: TransactionFiltersProps) {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await categoryAPI.getAll(); // ✅ no localhost
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          {activeFilterCount > 0 && (
            <span className="bg-indigo-600 text-white px-2.5 py-1 rounded-full text-xs font-medium shadow-sm">
              {activeFilterCount}
            </span>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <select
                ...
                value={filters.type}
                onChange={handleChange}
                className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-gray-900 appearance-none"
              >
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={handleChange}
              className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-gray-900"
            />
          </div>
        ...
        {activeFilterCount > 0 && (
          <div className="mt-5 pt-4 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="flex gap-1 flex-wrap">
                {filters.type && (
                  <span className="inline-flex items-center gap-1 text-xs bg-indigo-100 text-indigo-700 px-2 py-1 rounded-full">
                    {filters.type === 'income' ? <ArrowUp className="w-3 h-3" /> : <ArrowDown className="w-3 h-3" />}
                    {filters.type}
                  </span>
                )}
                {filters.category && (
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-xl transition-colors text-sm font-medium"
            >
