        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-4">
            <div
              ...
              style={{ backgroundColor: `${category.color}15`, boxShadow: `0 4px 12px ${category.color}30` }}
            >
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className={`text-xs font-medium px-2 py-1 rounded-full ${category.type === "income" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                  {category.type === "income" ? "Income" : "Expense"}
                </span>
                {category.isDefault && (
                  <span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full">Default</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-100">
            <div className="bg-linear-to-r from-gray-50 to-transparent rounded-xl p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-lg font-bold text-indigo-600">{format(category.budget.amount)}</span>
              </div>
              {category.budget.period && (
                <div className="flex items-center gap-2 text-xs text-gray-500">
                  ...
                  {category.budget.alert && (
                    <span className="ml-auto text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full">Alert on</span>
                  )}
                </div>
              )}
            </div>
