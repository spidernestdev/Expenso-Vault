...
import {
  ...
  Wallet, ChevronRight, Star, Home, FileText
} from "lucide-react";
    ...
    <aside className="hidden lg:block fixed left-0 top-0 w-64 h-screen bg-linear-to-b from-white to-gray-50/50 border-r border-gray-200 shadow-sm">
      ...
      <nav className="px-3 py-6">
        <div className="space-y-1">
          ...
          >
            <div className="flex items-center gap-3">
              <FileText className={`w-5 h-5 ${isChangelog ? "text-indigo-600" : "text-gray-500"}`} />
              <span className={`text-sm font-medium ${isChangelog ? "text-indigo-700" : ""}`}>Changelog</span>
            </div>
            {isChangelog && <ChevronRight className="w-4 h-4 text-indigo-600" />}
          </Link>
